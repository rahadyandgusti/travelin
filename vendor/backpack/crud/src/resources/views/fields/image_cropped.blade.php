  <div class="form-group col-md-12 image" data-preview="#{{ $field['name'] }}" data-aspectRatio="{{ isset($field['aspect_ratio'])?$field['aspect_ratio']:0 }}" data-crop="{{ $field['crop'] }}">
    ...
    <div class="row">
        <div class="col-sm-12" style="margin-bottom: 20px;">
            <img id="mainImage" src="{{ url((old($field['name']) ? old($field['name']) : (isset($field['value']) ? $field['path'].$field['value'] : (isset($field['default']) ? $field['path'].$field['default'] : '') ))) }}">
        </div>
        @if($field['crop'])
        <div class="col-sm-3">
                    ...
                    $uploadImage.change(function() {
                        ...
                        if (/^image\/\w+$/.test(file.type)) {
                            ...
                            fileReader.onload = function () {
                                $uploadImage.val("");
                                if(crop){
                                    $mainImage.cropper(options).cropper("reset", true).cropper("replace", this.result).cropper("setCropBoxData", { width: "1600", height: "1200" });
